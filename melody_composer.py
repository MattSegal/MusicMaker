...
class MelodyComposer(object):
    ...
    def compose(self,scaleNotes,duration):
        ...
        while counter < duration:
            ...
            finalOffset = chromatic.index(scaleNotes[int(noteOffset % scaleLen)]) + 12*finalOctaveOffset

            if finalOffset / 12 >= 6:
                finalOctaveOffset = 5
            melody.append((scaleNotes[int(noteOffset % scaleLen)],finalOctaveOffset,noteDuration, 30 + 20*int(tension/9) + randint(4,8) + meta_tension))
            counter = counter + 1
...
def compress(uncompressed):
    dict_size = 256
    dictionary = dict((chr(i), i) for i in iter(range(dict_size)))
    w = ""
    result = []
